...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    let isActive = false;
    let fadeOutTimer = 0;
    const FADE_OUT_DURATION = 2.0; // Seconds to fade out after mouse leaves
    ...
    function updateFrame() {
      ...
      // Handle fade-out when mouse leaves
      if (!isActive) {
        fadeOutTimer += dt;
        if (fadeOutTimer >= FADE_OUT_DURATION) {
          // Stop animation completely after fade duration
          return;
        }
      } else {
        fadeOutTimer = 0;
    ...
    function isInContainer(clientX: number, clientY: number): boolean {
    ...
    const handleTouchStart = (e: TouchEvent) => {
      ...
      if (!isInContainer(touch.clientX, touch.clientY)) return;
